...
// -> avatar(아바타 생성/닉네임) -> connecting(서버 접속 중) -> world(입장 완료)
export default function App() {
  ...
  async function handleJoin({ name, avatarPreset }) {
    setConnectError(null);
    setPhase("connecting");
    try {
      await connectToWorld({ name, avatarPreset });
      if (googleUser) {
        // 계정에 로그인된 상태면 이번에 고른 아바타/닉네임을 저장해서
        // 다음 접속 때 자동으로 불러오게 합니다 (실패해도 게임 진행엔 지장 없음).
        saveProfile({ name, avatarPreset });
      }
      setPhase("world");
    } catch (err) {
  ...
  if (phase === "avatar" || phase === "connecting") {
    // AvatarCustomizer를 계속 마운트 상태로 유지해서, 접속 실패 시에도
    // 선택했던 아바타/닉네임이 초기화되지 않도록 합니다.
    return (
      <>
        <AvatarCustomizer
          onJoin={handleJoin}
        ...
        />
        {phase === "connecting" && <LoadingScreen label="광장에 접속하는 중…" />}
      </>
    );
  }
